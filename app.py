...
# global variable
feedback_dataframe = pd.DataFrame(columns=['Name', 'Country', 'Feedback', 'Resume'])
...
def course_recommendation():
    ...
    comparison_path = r"C:\Users\sudhe\Projects\capstone\data\course_comparison.csv"
    ...
    with st.form('feedback_form'):
        ...
        if submitted_feedback:
            ...
            feedback_row = pd.DataFrame([[name, country, feedback, resume_filename]], columns=['Name', 'Country', 'Feedback', 'Resume'])
...
def admin():
    ...
    if not feedback_df.empty:
        ...
        for index, row in feedback_df.iterrows():
            cols = st.columns([1, 3, 2, 3])  # Adjust column widths as necessary
            with cols[0]:
